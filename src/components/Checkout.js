  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      if (e.response && e.response.status === 400) {
        ...
        enqueueSnackbar(
          "Could not place order. Check that the backend is running, reachable and working.",
          {
            variant: "error",
          }
        );
    ...
    <>
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh" key="fajdsfjksd">
            ...
            {!newAddress.isEditing && !newAddress.isAddingNewAddress && <Button
              color="primary"
              variant="contained"
              id="add-new-btn"
              size="large"
